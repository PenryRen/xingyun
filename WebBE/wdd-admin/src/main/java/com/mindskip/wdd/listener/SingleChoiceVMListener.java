 ...
 */
@AllArgsConstructor
public class SingleChoiceVMListener extends AnalysisEventListener<SingleChoiceVM> {

    private List<SingleChoiceVM> singleChoiceVMS;
    private ListenerParameter listenerParameter;
    private Boolean escapeContent;

    @Override
    public void invoke(SingleChoiceVM data, AnalysisContext context) {
        if (null == data) return;
        try {
            ...
            if (excelResult.getSuccess()) {
                ...
                Long existQuestion = listenerParameter.getQuestionService().checkLikeQuestion(new QuestionLike(null, questionEditRequestVM.getQuestionArchiveId(), QuestionTypeEnum.SingleChoice.getCode(), questionEditRequestVM.getTitle()));
