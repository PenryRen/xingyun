                                    ...
                                    if (ObjectUtils.isNotEmpty(metadata) && metadata.containsKey("name")) {
                                        ...
                                        if (StringUtils.isNotEmpty(name) && StringUtils.isNotEmpty(vmWare.getVmName()) && name.equals(vmWare.getVmName())) {
                                            ...
                                            if (item.containsKey("status")) {
                                                ...
                                                //拿到虚拟化ip
                                                if (status.containsKey("interfaces") && status.get("interfaces") instanceof JSONArray) {
...
//    @Scheduled(cron = "0 0 0 * * ?")
    ...
    public static void main(String[] args) {
        VmStatusEnum vmStatusEnum = VmStatusEnum.fromEnName("333");
        String code = vmStatusEnum.getCode();
