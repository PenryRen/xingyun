...
public class ForumController extends BaseApiController {
    ...
    public RestResponse<List<ForumArchiveVM>> list() {
        List<ForumArchive> rootTree = forumService.selectRootTree();
        List<ForumArchiveVM> rootVM = forumMapping.toForumArchiveVMList(rootTree);
        return RestResponse.ok(rootVM);
    }
    ...
    public RestResponse<PageInfo<ForumPageResponseVM>> page(@RequestBody ForumPageRequestVM model) {
        if (model.getForumArchiveId() != null) {
            ForumArchive rootForumArchive = forumService.getForumArchiveById(model.getForumArchiveId());
            List<ForumArchive> forumArchiveList = forumService.getForumArchiveByLevel(rootForumArchive.getLevel());
            List<Integer> forumArchiveIdList = forumArchiveList.stream().map(item -> item.getId())
                    .collect(Collectors.toList());
            model.setForumArchiveIdList(forumArchiveIdList);
        }
...
/*        List<Integer> forumArchiveIdList = pageInfo.getList().stream().map(d -> d.getForumArchiveId()).collect(Collectors.toList());
        ...
        PageInfo<ForumPageResponseVM> page = PageInfoHelper.copyMap(pageInfo, d -> {
            ...
            String content = StrUtil.sub(HtmlUtil.clear(d.getContent()), 0, 150);
...
/*            if (null != d.getForumArchiveId()) {
    ...
    public RestResponse<PageInfo<ForumCommentPageResponseVM>> commentPage(@RequestBody @Valid ForumCommentPageRequestVM model) {
        ...
        PageInfo<ForumCommentPageResponseVM> page = PageInfoHelper.copyMap(pageInfo, forumComment -> {
            ForumCommentPageResponseVM forumCommentPageResponseVM = commentRecursion(forumComment, userList);
            List<ForumCommentPageResponseVM> childCommentList = childForumCommentList.stream()
                    .filter(c -> c.getIdLevel().startsWith(forumComment.getIdLevel()))
                    .sorted(Comparator.comparing(ForumComment::getIdLevel))
                    .map(child -> {
                        ForumCommentPageResponseVM commentPageResponseVM = commentRecursion(child, userList);
                        userList.stream()
                                .filter(u -> u.getId().equals(child.getReplyUser()))
                                .findFirst()
                                .ifPresent(u -> {
                                    commentPageResponseVM.setReplyUserName(u.getUserName());
                                    commentPageResponseVM.setReplyRealName(u.getRealName());
                                });
                        return commentPageResponseVM;
                    }).collect(Collectors.toList());
            forumCommentPageResponseVM.setChild(childCommentList);
     ...
     * @return
     */
    private ForumCommentPageResponseVM commentRecursion(ForumComment forumComment, List<User> userList) {
        ForumCommentPageResponseVM forumPageResponseVM = forumMapping.toForumCommentPageResponseVM(forumComment);
