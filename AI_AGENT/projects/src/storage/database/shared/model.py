...
from sqlalchemy import BigInteger, Boolean, Column, DateTime, Double, Integer, Numeric, PrimaryKeyConstraint, Text, text, String, Float, ForeignKey, JSON, func, Index
...
class Student(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, comment="学生 ID")
...
class Question(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, comment="题目 ID")
    ...
    tags: Mapped[Optional[dict]] = mapped_column(Text, nullable=True, comment="标签 (JSON 格式)")
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False, comment="创建时间")
...
class ExamRecord(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, comment="考试记录 ID")
    student_id: Mapped[int] = mapped_column(Integer, ForeignKey('students.id'), nullable=False, comment="学生 ID")
...
class AnswerRecord(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, comment="答题记录 ID")
    exam_record_id: Mapped[int] = mapped_column(Integer, ForeignKey('exam_records.id'), nullable=False, comment="考试记录 ID")
    question_id: Mapped[int] = mapped_column(Integer, ForeignKey('questions.id'), nullable=False, comment="题目 ID")
...
class LearningProfile(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, comment="学情档案 ID")
    student_id: Mapped[int] = mapped_column(Integer, ForeignKey('students.id'), unique=True, nullable=False, comment="学生 ID")
    weak_points: Mapped[Optional[str]] = mapped_column(Text, nullable=False, comment="薄弱知识点 (JSON 数组)")
    mastered_points: Mapped[Optional[str]] = mapped_column(Text, nullable=False, comment="已掌握知识点 (JSON 数组)")
...
class StudentKnowledgeProgress(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, comment="进度 ID")
    student_id: Mapped[int] = mapped_column(Integer, ForeignKey('students.id'), nullable=False, comment="学生 ID")
