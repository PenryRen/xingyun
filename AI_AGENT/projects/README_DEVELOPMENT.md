...
> 本文原有“四模块/Supabase/智能组卷”内容是历史开发总结，已不代表可用功能。
> 当前考试分析和学情可视化由 WebBE 直接查询本地 MySQL 并用 Java 计算；Agent
> 不接触数据库、姓名、工号、完整答卷或登录凭据，只回答用户主动输入的问题。
> 智能组卷保持禁用，直至接入经过验证的本地题库。

# 麒麟系统教育平台 - 多智能体系统开发总结（历史）
