<template>
  <div class="app-container">
    <el-form :model="form" ref="form" label-width="150px">
      <el-form-item label="课件分类结构：" required>
    ...
    <el-dialog v-model="dialog.visible" append-to-body :close-on-click-modal="false" :show-close="false" center>
      <el-form :model="form" :rules="nameFormRules" ref="nameForm" label-width="120px">
        <el-form-item label="课件分类名称：" prop="name" required>
...
export default {
  ...
  data() {
    return {
      ...
      nameFormRules: {
        name: [{required: true, message: '课件分类名称不能为空'}, {
          max: 255,
          message: '长度小于255个字符'
        }]
      },
  ...
  methods: {
    ...
    treeNodeDrop(draggingNode, dropNode, dropType, ev) {
      let message = `此操作会将课件分类【${draggingNode.label}】放置到【${dropNode.label}】, 是否继续?`
    ...
    remove(node, data) {
      this.$confirm('此操作将永久删除该课件分类, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
