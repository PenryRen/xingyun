...
def generate_intelligent_paper(
    ...
    # 查询题库
    questions_response = client.table('questions').select('*').ilike('category', f'%{topic}%').execute()
    ...
    if len(filtered_questions) < question_count:
        filtered_questions = questions[:min(question_count * 2, len(questions))]
    ...
    if paper_type == "enhanced":
        ...
        if student_response.data:
            ...
            if profile_response.data:
                weak_points = profile_response.data[0]['weak_points']
                # 优先选择包含薄弱知识点的题目
                weak_questions = []
                other_questions = []
                for q in filtered_questions:
                    q_tags = q.get('tags') if isinstance(q.get('tags'), dict) else {}
                    q_category = str(q.get('category', '')) if q.get('category') else ''
                    # 检查题目是否包含薄弱知识点
                    is_weak_related = any(
    ...
    for idx, q in enumerate(selected_questions, 1):
        paper["questions"].append({
            ...
            "category": str(q.get('category', '')) if q.get('category') else '',
            "difficulty": float(q.get('difficulty', 0)) if q.get('difficulty') is not None else 0,
            "answer": str(q.get('answer', '')) if q.get('answer') else '',
            "explanation": str(q.get('explanation', '')) if q.get('explanation') else '',
...
def generate_enhanced_paper_by_weak_points(
    ...
    for weak_point in weak_points_list:
        # 搜索与薄弱点相关的题目
        questions_response = (client.table('questions')
                            .select('*')
                            .ilike('category', f'%{weak_point}%')
                            .execute())
        
        if questions_response.data:
            selected_questions.extend(questions_response.data)
    ...
    # 优先选择难度适中的题目
    # 按难度排序，选择中间难度的题目
    unique_questions.sort(key=lambda x: x['difficulty'])
