...
public class Seal {
    // 起始位置
    private int initBeginX;

    private int initBeginY;
    ...
    public void draw(Graphics2D g2d) {
        ...
        if (borderCircle != null) {
            drawCircle(g2d, borderCircle, initBeginX, initBeginY);
        }
        ...
        if (borderInnerCircle != null) {
            int x = initBeginX + borderCircleWidth - borderInnerCircle.getWidth();
            int y = initBeginY + borderCircleHeight - borderInnerCircle.getHeight();
            drawCircle(g2d, borderInnerCircle, x, y);
        }

        //5.画内环线圆
        if (innerCircle != null) {
            int x = initBeginX + borderCircleWidth - innerCircle.getWidth();
            int y = initBeginY + borderCircleHeight - innerCircle.getHeight();
        ...
        //8.画中心字
        drawFont(g2d, (borderCircleWidth + initBeginX) * 2, (borderCircleHeight + initBeginY) * 2, centerFont);

        //9.画抬头文字
        drawFont(g2d, (borderCircleWidth + initBeginX) * 2, (borderCircleHeight + initBeginY) * 2, titleFont);
    }
    ...
    private void drawArcFont4Circle(Graphics2D g2d, int circleRadius, SealFont font, boolean isTop) {
        ...
        //6.距离外圈距离
        int margin = font.getMargin() == null ? initBeginX : font.getMargin();
        ...
        for (int i = 0; i < textLen; i++) {
            ...
            g2d.setFont(f2);
            g2d.drawString(font.getText().substring(i, i + 1), (float) (circleRadius + thetaX + initBeginX), (float) (circleRadius - thetaY + initBeginY));
        }
    }
     ...
     * 绘制椭圆弧形文字
     */
    private void drawArcFont4Oval(Graphics2D g2d, SealCircle sealCircle, SealFont font, boolean isTop) {
        if (font == null) {
            return;
        }
        ...
        for (int i = 0; i < textLen; i++) {
            ...
            g2d.setFont(f2);
            g2d.drawString(c, x.intValue() + initBeginX, y.intValue() + initBeginY);
        }
    }
    ...
    private void drawFont(Graphics2D g2d, int circleWidth, int circleHeight, SealFont font) {
        ...
        if (fontTexts.length > 1) {
            ...
            //5.设置上边距
            float margin = initBeginX + (float) (circleHeight / 2 - y / 2);
            for (String fontText : fontTexts) {
     ...
     * 画圆
     */
    private void drawCircle(Graphics2D g2d, SealCircle circle, int x, int y) {
        if (circle == null) {
            return;
        }
    ...
    public boolean draw2(String pngPath) throws Exception {
        ...
        if (mainFont.getText().length() == 2) {
            if (stamp != null && stamp.trim().length() > 0) {
                ...
                marginW += Math.abs(rectangle.getCenterX()) * 2 + (mainFont.getSpace() == null ? initBeginX : mainFont.getSpace());
     ...
     * 画三字
     */
    private BufferedImage drawThreeFont(BufferedImage bi, Graphics2D g2d, SealFont font, int lineSize, int imageSize, int fixH, int fixW, boolean isWithYin) {
        fixH -= 9;
        ...
        if (isWithYin) {
            ...
            marginW += rectangle.getCenterX() * 2 + (font.getSpace() == null ? initBeginX : font.getSpace());
        } else {
            marginW += rectangle.getCenterX() * 2 + (font.getSpace() == null ? initBeginX : font.getSpace());
     ...
     * 画四字
     */
    private BufferedImage drawFourFont(BufferedImage bi, SealFont font, int lineSize, int imageSize, int fixH, int fixW) {
        int marginW = fixW + lineSize;
        //拉伸
        BufferedImage nbi = new BufferedImage(imageSize, imageSize, bi.getType());
        Graphics2D ng2d = nbi.createGraphics();
        ...
        g2d.drawString(font.getText().substring(2, 3), marginW, marginH);
        int oldW = marginW;
        marginW += Math.abs(rectangle.getCenterX()) * 2 + (font.getSpace() == null ? initBeginX : font.getSpace());
