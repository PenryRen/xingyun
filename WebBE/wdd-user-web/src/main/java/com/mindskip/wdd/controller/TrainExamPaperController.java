...
public class TrainExamPaperController extends BaseApiController {
    ...
    public RestResponse answerSubmit(@PathVariable @NotNull Long uid, @RequestBody EncryptKV encryptKV) {
        User user = getCurrentUser();
        RestResponse response = paperVerify(uid);
        if (response.getCode() != SystemCode.OK.getCode()) {
            return response;
        ...
        return new RestResponse(examPaperAnswerResult.getResultEnum().getCode(), examPaperAnswerResult.getMessage(), trainItemUser.getTrainId());
    }
